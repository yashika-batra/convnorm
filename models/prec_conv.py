...
class PreConv(_ConvNd):
    ...
    def preconditioning(self, cout, kernel):
        if (self.kernel_size[0]==1) or (self.kernel_size[1] ==1):
            ...
            f_input = torch.fft.rfft2(cout, norm="backward")
            with torch.no_grad():
                if self.training:
                    pad_kernel = F.pad(kernel, [0, final_size_y-self.kernel_size[1], 0, final_size_x-self.kernel_size[0]])
                    f_weight = torch.fft.rfft2(pad_kernel, norm="backward")
                    ...
                    V = self.running_V
            output = torch.fft.irfft2(compl_mul_2D(f_input, V), s=(final_size_x,final_size_y), norm="backward")
            return output
